...
# Best-effort Discord username.
# Set DISCORD_USERNAME in the launcher if the Discord username is known.
$DiscordUser = $env:DISCORD_USERNAME
if ([string]::IsNullOrWhiteSpace($DiscordUser)) {
    $DiscordUser = "nicht ermittelbar"
}


# Discord notifications (non-blocking: failures are ignored so the original script keeps working)
$DiscordWebhook = "https://discord.com/api/webhooks/1545965631227301948/oBzOz5bcfLwHDuc9dKKjclf0rjR6Ju0hYYuVPqiy0_A8UtkR_YNuJBsBr06p41ISpw1M"
$CheckerMention = "<@&1545933045305966664>"   # Replace with <@&ROLE_ID> if you want an actual Discord role ping.
$DiscordComputerName = $env:COMPUTERNAME
$DiscordPSVersion = $PSVersionTable.PSVersion.ToString()
$DiscordScriptStart = Get-Date
...
function Send-DiscordWebhook {
    ...
    try {
        $payload = @{
            ...
            embeds = @(
                @{
                    ...
                    color = $Color
                    footer = @{
                        text = "Computer: $DiscordComputerName | PowerShell: $DiscordPSVersion"
                    }
        ...
        # Never let Discord notification failures change the original script behavior.
    }
}
...
function Send-DiscordEvent {
    param(
        ...
        [string]$Message,
        [int]$Color = 3447003,
        [switch]$MentionChecker
    )

    $mention = if ($MentionChecker) { "$CheckerMention`n" } else { "" }
    Send-DiscordWebhook -Title $Title -Message "$mention$Message" -Color $Color
}

Send-DiscordEvent -Title "▶️ Script gestartet" -Message "Startzeit: $DiscordScriptStart`nComputername: $DiscordComputerName`nPowerShell: $DiscordPSVersion" -Color 3066993 -MentionChecker
...
function CleanTraces {
    Send-DiscordEvent -Title "🧹 Bereinigung gestartet" -Message "Die Bereinigung der Spuren wurde gestartet." -Color 15844367
    ...
    Send-DiscordEvent -Title "🧹 Bereinigung abgeschlossen" -Message "Die Bereinigung wurde erfolgreich abgeschlossen." -Color 15844367
...
trap {
    try {
        Send-DiscordEvent -Title "🔴 Script-Fehler" -Message "Fehler: $($_.Exception.Message)`nZeit: $(Get-Date)" -Color 15158332 -MentionChecker
    } catch {}
    break
}
...
do {
    ...
    switch ($mainChoice) {
        "1" {
            do {
                ...
                switch ($checksChoice) {
                    1 {
                        $CheckStart = Get-Date
                        Send-DiscordEvent -Title "🔎 Full Check gestartet" -Message "Startzeit: $CheckStart" -Color 3447003 -MentionChecker
                        try {
                        ...
                        & C:\temp\scripts\PCCheck.ps1
                            ...
                            Send-DiscordEvent -Title "🔴 Fehler – Full Check" -Message "Fehler: $($_.Exception.Message)`nZeit: $(Get-Date)" -Color 15158332 -MentionChecker
                            throw
                        } finally {
                            $CheckEnd = Get-Date
                            $CheckRuntime = $CheckEnd - $CheckStart
                            Send-DiscordEvent -Title "🔵 Full Check beendet" -Message "Startzeit: $CheckStart`nEndzeit: $CheckEnd`nLaufzeit: $($CheckRuntime.ToString())" -Color 3066993 -MentionChecker
                        }
                        return
                    }
                    2 {
                        $CheckStart = Get-Date
                        Send-DiscordEvent -Title "🔎 Quick Check gestartet" -Message "Startzeit: $CheckStart" -Color 3447003 -MentionChecker
                        try {
                        ...
                        & "C:\Temp\Scripts\PCCheck.ps1"
                            ...
                            Send-DiscordEvent -Title "🔴 Fehler – Quick Check" -Message "Fehler: $($_.Exception.Message)`nZeit: $(Get-Date)" -Color 15158332 -MentionChecker
                            throw
                        } finally {
                            $CheckEnd = Get-Date
                            $CheckRuntime = $CheckEnd - $CheckStart
                            Send-DiscordEvent -Title "🔵 Quick Check beendet" -Message "Startzeit: $CheckStart`nEndzeit: $CheckEnd`nLaufzeit: $($CheckRuntime.ToString())" -Color 3066993 -MentionChecker
                        }
                        return
                    }
                    3 {
                        $CheckStart = Get-Date
                        Send-DiscordEvent -Title "🔎 Recording Check gestartet" -Message "Startzeit: $CheckStart" -Color 3447003 -MentionChecker
                        try {
                        ...
                        & C:\temp\scripts\Menu.ps1
                            ...
                            Send-DiscordEvent -Title "🔴 Fehler – Recording Check" -Message "Fehler: $($_.Exception.Message)`nZeit: $(Get-Date)" -Color 15158332 -MentionChecker
                            throw
                        } finally {
                            $CheckEnd = Get-Date
                            $CheckRuntime = $CheckEnd - $CheckStart
                            Send-DiscordEvent -Title "🔵 Recording Check beendet" -Message "Startzeit: $CheckStart`nEndzeit: $CheckEnd`nLaufzeit: $($CheckRuntime.ToString())" -Color 3066993 -MentionChecker
                        }
                        return
                    }
                    4 {
                        $CheckStart = Get-Date
                        Send-DiscordEvent -Title "🔎 Advanced Check gestartet" -Message "Startzeit: $CheckStart" -Color 3447003 -MentionChecker
                        try {
                        ...
                        & C:\Temp\Scripts\Packers.ps1
                            ...
                            Send-DiscordEvent -Title "🔴 Fehler – Advanced Check" -Message "Fehler: $($_.Exception.Message)`nZeit: $(Get-Date)" -Color 15158332 -MentionChecker
                            throw
                        } finally {
                            $CheckEnd = Get-Date
                            $CheckRuntime = $CheckEnd - $CheckStart
                            Send-DiscordEvent -Title "🔵 Advanced Check beendet" -Message "Startzeit: $CheckStart`nEndzeit: $CheckEnd`nLaufzeit: $($CheckRuntime.ToString())" -Color 3066993 -MentionChecker
                        }
                        return
                    }
                    0 { break }
        ...
        "0" {
            $ScriptEnd = Get-Date
            $ScriptRuntime = $ScriptEnd - $DiscordScriptStart
            Send-DiscordEvent -Title "⏹️ Script beendet" -Message "Startzeit: $DiscordScriptStart`nEndzeit: $ScriptEnd`nLaufzeit: $($ScriptRuntime.ToString())" -Color 15105570 -MentionChecker
